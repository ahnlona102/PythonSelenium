...
class CreateAccountTest(BaseTest):
    ...
    def test_case7(self):
        try:
            ...
            self.capture_screenshot("login_testcase7_failure")
            raise
    ...
    def test_case8(self):
        try:
            ...
            self.capture_screenshot("login_testcase8_failure")
            raise
    ...
    def test_case9(self):
        try:
            ...
            user = User()
            user.set_username(test_data['username'])
            user.set_domain(test_data['domain'])
            user.set_email(test_data['username'] + "@" + test_data['domain'])
            user.set_password(test_data['password'])
            user.set_confirm_password(test_data['confirm_password'])
            user.set_passport(test_data['passport'])
            title_thanks = test_data['title_thanks']
            success_message = test_data['success_message']
            print(f"Test 9 - {threading.current_thread().name}")
            ...
            self.base_page.click_tab(RailwayTab.REGISTER)
            self.register_page.register_account(user)
            self.assertEqual(self.register_page.is_title_display(), title_thanks)
            self.base_page.switch_to_new_tab()
            self.base_page.navigate_to_url("mail.url")
            self.email_page.confirm_mess(ConfirmEmail.CONFIRM)
            self.assertTrue(self.register_page.is_message_displayed(success_message))
        except AssertionError:
            self.capture_screenshot("login_testcase9_failure")
            raise
